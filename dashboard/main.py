...
with st.sidebar:
    # Title
    st.title("Agiel Asyari")

    # Logo Image
    #st.image("/streamlit/agiel.jpeg")
    ...
    option = st.sidebar.radio('Menu', 
                      ("Daily Orders","Customer Spend Money","Order Items","Review Score", "Customer Demographic"))  
...
sns.barplot(y="product_count", x="product_category_name_english", data=sum_order_items_df.sort_values(by="product_count", ascending=False).head(5), palette="Blues_d", ax=ax[0])
...
sns.barplot(y="product_count", x="product_category_name_english", data=sum_order_items_df.sort_values(by="product_count", ascending=True).head(5), palette="Reds_d", ax=ax[1])
ax[1].set_ylabel(None)
ax[1].set_xlabel("Number of Sales", fontsize=30)
ax[1].invert_xaxis()
ax[1].yaxis.set_label_position("right")
ax[1].yaxis.tick_right()
ax[1].set_title("Least sold products", loc="center", fontsize=50)
...
sns.barplot(x=review_score.index, 
            ...
            palette=["#FFB000" if score == common_score else "#F8FF95" for score in review_score.index]
            )
...
file = Image.open(os.path.join(dir_name,"agiel.jpeg"))
st.sidebar.image(file, )
# Find me links
kaggle=' 🔍Find me on Linkedin [link](https://www.linkedin.com/in/agielasyari1/)'
st.sidebar.markdown(kaggle,unsafe_allow_html=True)
git=' 🔍 Find me on Git [link](https://github.com/agielasyari1)'
st.sidebar.markdown(git,unsafe_allow_html=True)
st.markdown("<p style='text-align:center;'>Copyright (C) Agiel Asyari 2023</p>", unsafe_allow_html=True)
